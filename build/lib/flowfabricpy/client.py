...
def flowfabric_list_datasets():
    ...
    :return: A list containing all the available datasets (each in JSON format)
    """
...
def flowfabric_get_dataset(dataset_id):
    """
    ...
    :return: Metadata for the dataset in JSON format
    """
...
def flowfabric_get_latest_run(dataset_id, verbose=False):
    ...
    :return: The latest run for the dataset in a DataTable
    """
...
def flowfabric_streamflow_query(dataset_id, feature_ids=None, start_time=None, end_time=None, issue_time=None, params=None, verbose=False, **kwargs):
    ...
    :param feature_ids: (Optional) The id(s) of the desired features
    :type feature_ids: list or None

    :param start_time: (Optional) The start time for the streamflow query
    :type start_time: str or None

    :param end_time: (Optional) The end time for the streamflow query
    :type end_time: str or None

    :param issue_time: (Optional) The issue time for the streamflow query
    :type issue_time: str or None

    :param params: (Optional) A list of parameters for the streamflow query
    :type params: dict or None

    :param verbose: (Optional) Provides extra information for debugging
    :type verbose: bool or None

    :return: Streamflow data from a dataset matching the parameters given/generated
    """
    auto_params = auto_streamflow_params(dataset_id)
    if auto_params is None:
        raise RuntimeError("auto_streamflow_params returned None. Unable to make a streamflow query.")
    if params:
        query_params = {k: v for k, v in params.items() if v is not None}
        for name in auto_params:
            query_params[name] = query_params[name] if name in query_params else auto_params[
                name] if name in auto_params else None
    else:
        query_params = dict()
    ...
    full_dataset = (
            ...
            and query_params.get("feature_ids") is None
    )
    if not is_zarr:
        if full_dataset:
            if query_params.get('issue_time') is None:
                query_params.update({'issue_time': "latest"})
    ...
    if full_dataset:
        ...
        if query_params['issue_time'] == "latest" and 'latest_issue_time' in est_resp.content:
...
def flowfabric_streamflow_estimate(dataset_id, feature_ids=None, start_time=None, end_time=None, issue_time=None, params=None, verbose=False, **kwargs):
    ...
    :return: A JSON object containing estimates on streamflow query matching the parameters given/generated
    """
    auto_params = auto_streamflow_params(dataset_id)
    if auto_params is None:
        raise RuntimeError("auto_streamflow_params returned None. Unable to make a streamflow estimate.")
...
def flowfabric_ratings_query(feature_ids, type="rem", format="arrow", verbose=False, **kwargs):
    ...
    :param feature_ids: The id(s) of the desired features
    :type feature_ids: list

    :param type: (Optional) The type of the ratings
    :type type: str or None

    :param format: (Optional) The format of the ratings
    :type format: str or None

    :param verbose: (Optional) Provides extra information for debugging
    :type verbose: bool or None

    :return: A response (parsed based on format) containing the REM ratings for given features
    """
    ...
    if format == "arrow" and resp.headers["Content-Type"] == 'application/vnd.apache.arrow.stream':
...
def flowfabric_ratings_estimate(feature_ids, type="rem", format="json", verbose=False, **kwargs):
    ...
    :return: An estimate of REM ratings data size for given features
    """
    params = dict(feature_ids=feature_ids, type=type, format=format)
    if kwargs:
        params.update(kwargs)
...
def flowfabric_stage_query(dataset_id, params=None, verbose=False, **kwargs):
    ...
    :return: Stage data for the given dataset
    """
    if params is None:
        params = dict(kwargs)
    if params is None:
...
def flowfabric_inundation_ids(params=dict, verbose=False):
    """
    Return inundation polygon grid ids

    :param params: A list of parameters for the streamflow query
    :type params: dict
...
def flowfabric_healthz(token=None, verbose=False):
    ...
    :return: Health metadata for the API
    """
    if token is None:
...
def get_bearer_token(force_refresh=False):
    ...
    :param force_refresh: (Optional) Force refresh cached token
...
def normalize_time(obj, is_start=True):
    """
    Helper function to normalize a datetime object. Do not call directly.
    :param obj:
    :param is_start:
    :return:
    """
...
def is_zarr_dataset(meta):
    """
    Helper function to determine if a dataset is a Zarr dataset. Do not call directly.
    :param meta:
    :return:
    """
    return (
